...
Execution flow:
  1. Start FastAPI/Uvicorn server on 127.0.0.1:48157 in a background daemon thread
  2. Wait up to 5 s for the server to be ready (polling /api/status)
  3. Open a native OS desktop window via pywebview
  4. Block until the user closes the window
  5. Daemon thread exits automatically with the process
"""
import sys
import os
import time
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="OPTO-PROFIT Desktop Launcher")
    parser.add_argument("--dev", action="store_true", help="Launch pointing to hot-reloading Vite server")
    ...
    target_url = "http://localhost:5173" if args.dev else BASE_URL

    # Open the desktop window
    logger.info("Opening desktop window pointing to %s …", target_url)
    try:
        ...
        # Fallback: pywebview failed (missing WebView2, missing DLLs, ImportError, etc.)
        ...
        # Keep the server alive until user closes the browser tab / presses Ctrl+C
