...
Since the application operates in a strict offline, air-gapped environment, traditional web application firewalls (WAF) are not required.
1.  **Network Isolation**: Ensure the host machines are disconnected from external networks to prevent remote exfiltration.
2.  **OS-Level Firewalls**: Restrict incoming traffic to the localhost loopback on port 8000.
3.  **Audit Trail Monitoring**: Feed `/api/status` probes and error logs into local log parsers to alert system engineers upon catching anomalous spike waves in user login failures or computational timeouts.
